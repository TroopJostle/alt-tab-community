...
enum PreferencesPersistenceCheck {
    /// Every suite AltTab persists to. `UserDefaults.standard` is keyed by the bundle id; the other is
    /// an explicit suite (usage stats). System suites we only read (e.g. `com.apple.Finder`) are out of
    /// scope. (The former license suite was removed when Pro was unlocked for everyone.)
